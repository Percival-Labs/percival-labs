/**
 * Roadmap Worker -- Dual-format logger (JSON + human-readable)
 *
 * Mirrors Oracle/Sentry logger pattern. Writes to stdout and log files.
 */
...
const minLevel: LogLevel =
  (process.env.ROADMAP_WORKER_LOG_LEVEL as LogLevel) || 'info';

let logFileReady = false;
const logFilePath = join(PATHS.logDir, 'roadmap-worker.log');
const jsonLogPath = join(PATHS.logDir, 'roadmap-worker.jsonl');
