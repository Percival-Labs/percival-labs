...
function buildSystemPrompt(identity: AgentIdentity, memoryContext: string): string {
  ...
  if (workspacePath) {
    parts.push(
      ...
      `Key locations:`,
      `- ${workspacePath}/apps/ — Application source code (agents, discord, terrarium, website, vouch, etc.)`,
      `- ${workspacePath}/packages/ — Shared packages (vouch-db, shared, agent-memory, db)`,
